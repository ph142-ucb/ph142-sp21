...
### Instructions 
* Due date: Friday, Jan 22 at 10:00pm.
...
analyze data. 
- If you've never programmed before, that is okay. We will teach you the basics and ensure you know how to do everything you need to know for this course.
...
knitr::include_graphics("lab01-rstudio-panes.png")
```
...
**Creating an object**
If you would like to save something (a number, a vector, a dataset etc) for use later, you can save it as an object.  You can think of this like creating a jar where you will put something and giving it a label.  

For example typing the code `a <- 2` into the console.  This stores the value `2` in the object `a`. You can also type `a <- 1 + 1`, and this does the same thing, since the evaluation of the right-hand side happens before assignment to the variable.  Notice that when you create an object, R does not respond, but if you then type the object name, R will respond with the contents of the object.  

**Code chunks**
In the R markdown files you are given, we will often provide code chunks, these are little sections of code set off from the text of the file with ```{r} at the beginning and ``` at the end.  In Rstudio you can see that the code chunks appear in a grey background.  You should also notice the green arrows in the upper right hand side of each chunk.  If you click the arrow pointing to the right, R will submit everything in the "chunk" and you will see the results just below the chunk.

- **Exercise 3:** In the code chunk below, use the assignment operator (`<-`) to store your answer in an object called `monthly_wage`. When assigning variables, the stuff on the right-hand side of the arrow `<-` is evaluated first, and then assigned to the object specified by the variable name on the left of the arrow.

```{r monthly-wage}
monthly_wage <- "YOUR CODE HERE"
```

You can also save a series of numbers in an object.  For example: `heights <- c(5,6,2,9,6)`
will save a set of values in the object "heights". The `c()` tells R to "combine" the numbers into a list, known in R as a **vector**.

**Pane 2. Environment/History/Connections:** What is in the environment tab right now? It is letting you know about the **objects** that we saved in the **Environment**. When we load in a data set, we will also be able to see its object here too. For data sets, there is an icon we can push to view all the data. Now click the History tab. What is it? Click a line of code and hit the "To Console" or "To Source" buttons. What happens? The History is a great way to recall a line of code that you may have typed directly in the console but hadn't saved in the code editor and would like to use again. We will ignore the connections tab.

If you ran the code chunk above, you should see `monthly_wage` as an object in the Environment window.  

**Types of variables in R **
Notice in the Environment tab that you can see a descrption of each object that has been saved.  
...
knitr::include_graphics("lab01-source-code-chunk-description2.png")
```

- **Exercise 5:** Read the text in the practice script you created. Click the button labeled **Knit**. Save the file name as "PracticeScript". Compare the file that is generated when you hit "Knit" (it should open in a pop-up window) with the script shown in the code editor. 
...
- **Exercise 6:** Delete lines 5 through 28 of the practice script markdown file. This should leave only the header information (the top four lines including the "---" at the top and bottom). Knit your file and see what appears in the Viewer window. 
- **Exercise 7:** Update the title of your file (line 1) to "Report on wages" by typing into the source. Then, starting on line 6 write: "To calculate my monthly wage, I multiplied the hourly rate by the number of hours worked and multiplied this product by the number of weeks in a month. Finally, I multiplied by 80% to account for taxes." Now, Knit your file again.
- **Exercise 8:** Add a code chunk to the file at line 10. To do this, click the place in the template you'd like to add your chunk, and then click the **Insert** button and choose R from the dropdown menu. The keyboard shortcut for this is `option + cmd + i` on Mac OS and `Ctrl + Alt + i` on PC. 
- **Exercise 9:** Write the code `monthly_wage <- 20 * 15 * 4 * 0.8` inside the code chunk. Name the code chunk "calculate-wage" (see the second figure to remind yourself how to name the code chunk). 
- **Exercise 10:** On the next line (inside the code chunk) write `monthly_wage`. When we execute this line, we are asking R to display the value that we assigned to the variable `monthly_wage`. Execute the code chunk by pressing its green arrow or placing your cursor on the line of code and hitting `cmd + enter` on Mac or `Ctrl + enter` on PC.
...
On Datahub, we have already installed the package `dplyr` and `ggplot2` for us to use during the course. We just need to load the packages using the `library()` function:
...
You can identify a function because it will always end with a set of round brackets `()`. Another name for **function** is **command**. `library(dplyr)` is a **function** call with one argument, `dplyr`. What is inside its round brackets is an **argument** of the **function**. Many functions take multiple arguments. Here, we are using the function to load the `dplyr` package on the first line and the `ggplot2` package on the second line.
...
We are introducing an autograder for this class. Basically, we've written a series of test cases and checkpoints that test the correctness of your code and allows you to check your work and receive automated real-time feedback on your answers. 

Now, let's try solving a problem. In your assignments, you would be given a prompt and asked to code the answer and assign it to an object. Do not change the name of this object. **For this problem, set `p1` to a mathematical expression that evaluates to 5.** You'll often get starter code that will look like this:
...
You'll also run into problems that will ask you to explain your answer in a couple of sentences, like the problem below. Simply replace the line `[TODO: YOUR ANSWER HERE]` with your answer. You won't see an autograder check for these free-response questions, since they'll be graded by humans.

**Problem 4: What are you looking forward to most in this course?**

I am looking forward to [TODO: YOUR ANSWER HERE]


Now, at the end of the assignment, you should go back through and run each code chunk to be sure you passed all of your tests. On this assignment, that only means that `p1`, `p2`, and `p3` should all equal 5. One way to check that works well is to click the grey downward facing arrow in the right corner of the code chunk. This will run all previous code chunks so all of your anwsers are up to date.
...
Run these next cells to correct all your answers.
```{r correct-p1}
p1 <- 5
...
The autograder is meant to be used as a sanity check for your work, not as an absolute check of correctness. This is because, in the real world, you won't have an autograder to check your analysis/code... unless you write your own test cases (which is recommended, but we won't be covering that in this course). However, **programming is difficult**, and we want to provide you with the best resources to succeed so we will write all the test cases in labs. Since homework is not being collected for credit, we will also write all the test cases for homework. Our goal is that you won't rely too much on the autograder for the homeworks - you can try out a few things and try to intuitively solve the problems. Ask lots of questions - we're excited for you to start the journey of being a data analyst.

Since **labs** have an emphasis on introductory learning, all test cases will be provided. This means, if you pass the test cases given in the assignment, you'll receive full credit on the assignment. Because of this design, labs will be graded **all or nothing,** which means you should pass all tests before submitting.
...
We know we're also not perfect, and sometimes we mess up the test cases on the autograder, so a correct answer might get graded as incorrect (we hope this is rare), so we highly encourage you to check your assignment promptly after grades are released to make sure you get the score you earned. If you suspect the autograder might be incorrect, you can submit a regrade on Gradescope. Per the regrade policy, **all assignment regrades will be open for 3 school days after score release** (scores released anytime Jan 1 means regrades close Jan 4 night, 11:59pm).


### Resources
We will do our best to introduce and demonstrate all of the code you will need for this class.  If you would like to study more on your own, there are many many resources available online.  If you have questions about R or get stuck, please visit office hours or post to Piazza.
...
cd; cd ph142-sp21/lab/lab01; python3 turn_in.py
